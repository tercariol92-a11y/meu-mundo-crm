...
export default async function handler(req:Req,res:Res){
  try{
    ...
    const target=new URL(`/api/whatsapp/media/${encodeURIComponent(messageId)}`,new URL(serviceUrl).toString());target.searchParams.set('sessionId',sessionId);target.searchParams.set('storagePath',storagePath);

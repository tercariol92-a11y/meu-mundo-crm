...
export function buildCommonSaleNfeXml(input: NfeCommonSaleXmlInput) {
  ...
  const totalFreight = Number(input.freight || 0);
  const totalInvoice = totalProducts + totalFreight;
  let allocatedFreight = 0;
  const freightByItem = input.items.map((item, index) => {
    if (!totalFreight) return 0;
    if (index === input.items.length - 1) return Number((totalFreight - allocatedFreight).toFixed(2));
    const share = totalProducts > 0 ? Number((totalFreight * ((item.quantity * item.unitValue) / totalProducts)).toFixed(2)) : 0;
    allocatedFreight += share;
    return share;
  });

...
const failureGuidance = (code?: string, message?: string) => {
  const normalized = `${code || ''} ${message || ''}`.toUpperCase();
  if (normalized.includes('E0188') || normalized.includes('CNPJ DO TOMADOR')) return {
    title: 'Corrigir o CNPJ do tomador',
    detail: 'O CNPJ enviado não passou na validação oficial. Confira os 14 números e os dois dígitos verificadores no cadastro do cliente.',
  };
  if (normalized.includes('DPS_XSD_INVALID')) return {
    title: 'Corrigir os dados destacados da DPS',
    detail: 'Um campo não atende ao formato oficial. Abra a correção para ver os dados cadastrais e fiscais usados nesta cobrança.',
  };
  if (normalized.includes('SEFIN_REJECTED')) return {
    title: 'Revisar os dados e revalidar',
    detail: 'A SEFIN não retornou um código específico. Confira tomador, endereço, município/IBGE e parâmetros do serviço antes de liberar nova tentativa.',
  };
  return {
    title: 'Revisar a cobrança',
    detail: message || 'Confira os dados do cliente e do contrato antes de tentar novamente.',
  };
};
...
export default function RecurringBillingQueue({ user, contracts, clients, config, credentialsRef, environment = 'producao', onCompleted }: Props) {
  ...
  const revalidateRejected = async () => {
  ...
  const saveAndRevalidate = async () => {
    ...
    try {
      ...
      if (candidate.status === 'PRONTO_PARA_EMITIR') {
        setEditing(null);
        setSelected(current => current.includes(candidate.id) ? current : [...current, candidate.id]);
        setMessage('Cadastro corrigido e validado. A cobrança foi selecionada e está pronta para uma nova emissão manual.');
      }
  ...
  const processSequentially = async () => {
    ...
    let errorCount = 0;
    let billedAmount = 0;
    const failures: Array<{ id: string; contractNumber: string; clientName: string; code?: string; message: string }> = [];
    for (const billing of chosen) {
      const client = clients.find(item => item.id === billing.clientId);
      if (!client) {
        const failure = { id: billing.id, contractNumber: billing.contractNumber, clientName: billing.clientName, code: 'CLIENT_NOT_FOUND', message: 'Cliente/tomador não localizado no cadastro.' };
        await updateRecurringBilling(billing.id, { status: 'PENDENCIA_CADASTRAL', missingFields: [failure.message], sefinError: { code: failure.code, message: failure.message } });
        failures.push(failure); errorCount += 1; continue;
      }
      try {
  ...
  return <div className="mb-5 rounded-2xl border border-blue-200 bg-blue-50/30 overflow-hidden">
    ...
    {editing && <div className="fixed inset-0 z-[180] flex items-center justify-center bg-black/45 p-4"><div className="max-h-[92vh] w-full max-w-4xl overflow-y-auto rounded-2xl bg-white shadow-2xl"><div className="sticky top-0 z-10 flex items-center justify-between border-b bg-white p-5"><div><h2 className="font-extrabold text-slate-900">Pendências para emissão da NFS-e</h2><p className="text-xs text-slate-500">{editing.clientName} · {editing.contractNumber}</p></div><button onClick={() => setEditing(null)}><X size={20}/></button></div>
      <div className="p-5">{editing.sefinError && <div className="mb-4 rounded-xl border border-rose-200 bg-rose-50 p-4 text-sm text-rose-900"><div className="font-extrabold">Erro retornado pela SEFIN {editing.sefinError.code ? `· ${editing.sefinError.code}` : ''}</div><div className="mt-1">{editing.sefinError.message}</div><div className="mt-2 text-xs"><strong>Orientação:</strong> {failureGuidance(editing.sefinError.code, editing.sefinError.message).detail}</div></div>}<div className="mb-5 space-y-2 rounded-xl border p-4">{issues.length ? issues.map(issue => <div key={issue.key} className="flex items-center gap-2 text-sm text-rose-700"><AlertTriangle size={15}/><span>{issue.label}</span><span className="ml-auto text-[10px] uppercase text-slate-400">Salvar em {issue.origin === 'cliente' ? 'Dados do cliente' : issue.origin === 'contrato' ? 'Dados fiscais do contrato' : 'Configuração fiscal'}</span></div>) : <div className="flex items-center gap-2 text-emerald-700"><CheckCircle2 size={16}/> Todos os dados locais estão válidos. Salve para liberar uma nova tentativa manual.</div>}</div>
        ...
        <div className="mt-5 flex justify-end gap-2"><button onClick={()=>setEditing(null)} className="rounded-xl border px-4 py-2 text-xs font-bold">CANCELAR</button><button disabled={saving} onClick={()=>void (editing.status === 'REJEITADA' ? revalidateRejected() : saveAndRevalidate())} className="rounded-xl bg-blue-600 px-5 py-2 text-xs font-bold text-white disabled:opacity-50">{saving ? 'SALVANDO...' : 'SALVAR E VALIDAR NOVAMENTE'}</button></div></div></div></div>}
  </div>;
}

...
test('CTR-9075 generates one ready billing for the competence', () => {
  ...
  const billing = buildRecurringBilling('empresa1', contract, client, '2026-08', 'producao');
...
test('missing taker data blocks fiscal emission without transmitting', () => {
  const billing = buildRecurringBilling('empresa1', contract, { ...client, codigoIbge: '', cnpj: '' }, '2026-08', 'producao');
  assert.equal(billing.status, 'PENDENCIA_CADASTRAL');
  assert.ok(billing.missingFields.includes('CPF/CNPJ'));
  assert.ok(billing.missingFields.includes('código IBGE'));
});
